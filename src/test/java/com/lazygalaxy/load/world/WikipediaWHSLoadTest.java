...
import com.lazygalaxy.load.world.wikipedia.WHSByCountryProcessor;
...
public class WikipediaWHSLoadTest extends TestCase {
	private static final Logger LOGGER = LogManager.getLogger(WikipediaWHSLoadTest.class);
	...
	public void testParent() throws Exception {
		...
		assertEquals("com.lazygalaxy.load.world.wikipedia.WHSParentProcessor", pages.get(0).processor);
		assertEquals(
		...
		assertEquals("com.lazygalaxy.load.world.wikipedia.WHSByCountryProcessor", pages.get(1).processor);

		LOGGER.info(pages.get(pages.size() - 1));
		assertEquals("listofworldheritagesitesinzimbabwe", pages.get(pages.size() - 1).id);
		assertEquals("com.lazygalaxy.load.world.wikipedia.WHSByCountryProcessor",
				pages.get(pages.size() - 1).processor);
	}
	...
	public void testByCountry() throws Exception {
		...
		expectedPage.processor = "com.lazygalaxy.load.world.wikipedia.WHSByCountryProcessor";
		expectedPage.mustUpdate = true;
		assertPage(expectedPage, pages.get(0));

		LOGGER.info(pages.get(1));
		expectedPage = new WikipediaPage("Palace at Caserta", new String[] {});
		expectedPage.image = new Image("arowofwaterbasinsleadingtoalargepalacebuilding",
